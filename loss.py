...
def get_loss(loss_type, num_class, ignore_index, device, upper_bound=1.0):
    """
    ...
    if loss_type == "weighted":
        criterion = ImageBasedCrossEntropyLoss2d(
            classes=num_class, size_average=True,
            ignore_index=ignore_index,
            device=device,
            upper_bound=upper_bound).to(device)
    elif loss_type == "relax":
        criterion = ImgWtLossSoftNLL(classes=num_class,
                                     ignore_index=ignore_index,
                                     device=device,
                                     upper_bound=upper_bound).to(device)
    else:
...
class ImageBasedCrossEntropyLoss2d(nn.Module):
    ...
    def __init__(self, classes, device, weight=None, size_average=True, ignore_index=255,
    ...
    def forward(self, inputs, targets):
        ...
        if self.batch_weights:
            ...
            self.nll_loss.weight = torch.Tensor(weights).to(self.device)
            #print(weights)
        loss = 0.0
...
class ImgWtLossSoftNLL(nn.Module):
    ...
    def __init__(self, classes, device, ignore_index=255, weights=None, upper_bound=1.0,
                 norm=False):
