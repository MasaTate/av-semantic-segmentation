...
def make_dataset(root, mode, tracks=[3, 8]):
    ...
    if mode == 'train':
        for sc in range(1,115):
            ...
            # sound making map
            binary_mask_path = os.path.join(root, img_dir_name,'binary_masks_full/')
            
            mask_postfix = '_gtDLab_labelId.png'
            for it_full in glob.glob(check_audioImg_path+"*.npy"):
                print(it_full)
                it = it_full.split('/')[-1].split('.')[0]
                if it_full in audioDict.item()[sc]:
                    item = (os.path.join(img_path, "*_"+it+".png"), os.path.join(mask_path, "*"+it+mask_postfix), os.path.join(binary_mask_path, "*"+it+mask_postfix), os.path.join(audioImg_path, it+".npy"), os.path.join(audioImg_path6, it+".npy"), mode)
                    items.append(item)
...
class SoundCityscapesAuth(data.Dataset):
    ...
    **Parameters:**
    ...
    def __getitem__(self, index):
        ...
        """
        img_path, mask_path, bin_mask_path, spec_path_1, spec_path_2 = self.items[index]
